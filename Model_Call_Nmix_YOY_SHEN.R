### Brook trout YOY abundance model for SHEN adapted from Kanno et al. (2016) Freshwater Biology
# NP Hitt <nhitt@usgs.gov> 3 Feb 2016

# Load working directory & libraries
setwd("C:/Users/nhitt/Desktop/Nmix_SHEN")
...
# Fish count data
load("Data_SHEN_FishCountAr.RData")
# Seasonal climate data
load("Data_SHEN_SeasonalClimateStd.RData") # Standardized to site-specific mean=0 and sd=1
# Site covariate data
load("Data_SHEN_SiteCovsStd.RData") # Standardized to dataset-level mean=0 and sd=1
# Detection covariate data
load("Data_SHEN_DetectionCovsStd.RData") # Standardized to dataset-level mean=0 and sd=1

# Set up data structure
nSites=157; nYears=31; nCovs=6; nPasses=3

# Adding 1 fish to abundances to avoid log(0) problem
YOYFish <- YOYFish + 1

# Bundle data for YOY model
dat <- list(nSites=nSites, nYears=nYears, nCovs=nCovs, y=YOYFish,
...
save(out1, out2, file="ResultsRaw_YOY_SHEN_3Feb16.RData")
...
save(N.est, y.est, file="Result_YOYEst_SHEN_1Nov15.RData")

# gelman.diag(out1)

#########################################################
# Plot histograms for seasonal climate covariates
pdf("Result_EnvHist_YOY_SHEN_3Feb16.pdf")
